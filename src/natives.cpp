// this is the single translation unit that pulls in the pawn amx shims
...
int dotenv::RegisterNatives(AMX* amx)
{
	return amx_Register(amx, kNatives, -1);
}

void dotenv::WireAmx(void* const* exports)
{
	std::array<void*, NUM_AMX_FUNCS> funcs;
	for (int i = 0; i < NUM_AMX_FUNCS; ++i)
		funcs[i] = exports[i];
	setAmxFunctions(funcs);
}